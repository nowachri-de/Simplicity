...
describe('Matt Mazure Test', function () {
    it('It should add helper functions to Kernel', function () {
        Kernel.addFunction(function sigmoidActivation(i=0.) {
            return 1 / (1 + exp(-i));
        });

        Kernel.addFunction(function derivativeSigmoid(out=0.) {
            return out - pow(out, 2);
        });
